    ...
    <>
      ...
      >
        <div className="custom-header mb-6 text-center border-b pb-4">
          <h1 className="text-xl font-bold">كشف المخازن</h1>
        </div>

        <div className="header-info">
          <div className="info-right">
            <div><strong>بواسطة:</strong> {reportData.generatedBy}</div>
            <div><strong>عدد المخازن:</strong> {reportData.count}</div>
          </div>
          ...
          <table className="items-table">
            ...
            <tbody>
              {reportData.items.map((item, index) => (
                <tr key={index}>
                  <td>{index + 1}</td>
                  <td className="text-right font-semibold">{item.name}</td>
                  <td>{item.location}</td>
                  <td>{item.storekeeper}</td>
                  <td dir="ltr">{item.phone}</td>
                  <td>{item.status}</td>
                  <td dir="ltr">{new Date(item.createdAt).toLocaleDateString('en-GB')}</td>
                  <td>{item.details}</td>
                </tr>
              ))}
            </tbody>
