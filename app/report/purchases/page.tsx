            ...
            <tbody>
              {reportData.items.map((item, index) => (
                <tr key={index}>
                  ...
                  <td>{item.typeofbuy}</td>
                  <td>{item.typeofpayment}</td>
                  <td>{item.numberofpurchase}</td>
                  <td style={{ textAlign: "right" }}>{item.totalpurchaseiqd.toLocaleString()}</td>
                  <td style={{ textAlign: "right" }}>{item.totalpurchaseusd.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                  <td dir="ltr" style={{ textAlign: "center" }}>{formatDate(item.datetime)}</td>
                  <td style={{ fontSize: "10px" }}>{item.details}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="totals-row">
                <td colSpan={5} style={{ textAlign: "left", fontWeight: "bold" }}>المجموع الكلي:</td>
